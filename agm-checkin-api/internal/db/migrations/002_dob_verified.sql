...
-- Backfill. A competitor counts as verified only if they were marked validated
-- AND we actually hold a date of birth for them — claiming we verified a date we
-- do not have is worse than asking for ID once more at the desk. Go's zero time
-- is the "no DOB on file" sentinel here, not NULL.
WITH verified AS (
    SELECT c.id,
...
-- Re-derive the legacy booleans from the new field so the two encodings agree
-- everywhere. Without this the 57 rows that were (true, true), the one corrupt
-- (false, false) row, and the 10 we just un-verified would keep contradicting
-- dob_verified_at, and a rollback would resurrect the old policy on exactly the
-- rows the new one was meant to fix. The API writes both columns the same way,
-- so this invariant holds from here until migration 004 drops them.
UPDATE competitors
